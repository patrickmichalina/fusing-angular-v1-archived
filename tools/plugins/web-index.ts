...
export class WebIndexPluginClass {
  ...
  producerEnd(producer: BundleProducer) {
    ...
    const appElement: Dependency = this.opts.appElement
      ? {
          element: this.opts.appElement.name,
          content: this.opts.appElement.innerHTML
        }
      : (undefined as any)
          ...
          return {
            ...
            attributes: {
              src: bundle.context.output.folderFromBundleName
                ? `/${bundle.context.output.folderFromBundleName}/${
    ...
    const startingHtml = this.opts.startingDocumentPath
      ? readFileSync(
          join(producer.fuse.context.appRoot, this.opts.startingDocumentPath),
          'utf-8'
        )
      : undefined

    let html = depTransformer.applyTransform(
      deps,
      startingHtml,
      this.opts.appElement && this.opts.appElement.name
    )
    ...
    const finalHtml =
      typeof this.opts.postProcess === 'function'
        ? this.opts.postProcess(html)
        : html

    producer.fuse.context.output.write(
      this.opts.target || 'index.html',
      finalHtml,
      true
    )
  }
...
export const WebIndexPlugin = (
  options: WebIndexPluginOptions = { bundles: [], additionalDeps: [] }
) => new WebIndexPluginClass(options)
      ...
      new Set(
        Array.from(
          doc.window.document.querySelectorAll(
            `script, link, meta, base, ${appName}`
          )
        ).map(a => hash(a.outerHTML))
      )
    )
