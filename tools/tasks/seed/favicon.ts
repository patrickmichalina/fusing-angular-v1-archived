...
Sparky.task(taskName(__filename), () => {
  return new Promise((resolve, reject) => {
    ...
    favicons(favicon.src, favicon.config, (error: any, response: any) => {
      if (error) {
      ...
      const imagePromises = (response.images as Array<{ name: string, contents: Buffer }>).map(image =>
        new Promise((resolve1, reject1) => {
          return writeFile(`./${BUILD_CONFIG.clientAssetsDir}/favicons/${image.name}`, image.contents, (err: any) => {
            if (err) reject1(err)
      ...
      const filePromises = (response.files as Array<{ name: string, contents: string }>).map(file =>
        new Promise((resolve2, reject2) => {
          ...
          writeFile(`./${BUILD_CONFIG.clientAssetsDir}/favicons/${file.name}`, file.contents, (err: any) => {
            if (err) reject2(err)
      ...
      return Promise.all<any>([...imagePromises, ...filePromises]).then(() => {
        ...
        readFile(filePath, 'utf-8', (err, data) => {
          ...
          const existingHtmlHead = (existingHtml.window.document.head as HTMLHeadElement)
          const newHtmlHead = (newHtml.window.document.head as HTMLHeadElement)
          const children = Array.from(newHtmlHead.children)
          const toReset = existingHtmlHead.querySelectorAll('[data-inj-fav]')
          Array.from(toReset).forEach(a => a.remove())
          children.forEach((a: HTMLElement) => a.setAttribute('data-inj-fav', ''))
          children.forEach(el => existingHtmlHead.appendChild(el))
          writeFile(filePath, existingHtml.serialize(), { encoding: 'utf-8' }, writeErr => {
