...
export class HttpCookieInterceptor implements HttpInterceptor {
  ...
  intercept(
    ...
    const whitelistedDomains = this.injector.get(
      COOKIE_HOST_WHITELIST
    ) as ReadonlyArray<string>
    const url = new URL(req.url)

    if (whitelistedDomains && !whitelistedDomains.some(a => a === url.hostname))
      return next.handle(req)

    if (this.ps.isServer) {
      const serverRequest = this.injector.get(REQUEST) as express.Request

      const cookieString = Object.keys(serverRequest.cookies || {}).reduce(
        (accumulator, cookieName) => {
          return `${accumulator}${cookieName}=${
            serverRequest.cookies[cookieName]
          };`
        },
        ''
      )
